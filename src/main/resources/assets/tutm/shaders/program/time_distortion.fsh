...
void main() {
    ...
    // 1. Chromatic Aberration (Matching time_spatial mode 1)
    // Scale shift by Intensity (up to 0.02, slightly stronger than base mode 1)
    float shift = Intensity * 0.02 * dist;
    ...
    // 2. Saturation Boost (Matching time_spatial mode 1)
    float luma = dot(color, vec3(0.299, 0.587, 0.114));
    float saturationFactor = 1.0 + (0.4 * Intensity); // Boost saturation up to 1.4
    color = mix(vec3(luma), color, saturationFactor);

    // 3. Vignette (Matching time_spatial mode 1)
